...
const AttemptCard = ({ attempt, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-xl p-5 border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center gap-6 hover:shadow-md transition-shadow group ${onClick ? "cursor-pointer" : ""
        }`}
    >
      {/* DATE */}
      ...
      <div className="flex-1">
        ...
        <div className="flex items-baseline gap-1">
          <span
            className={`text-xl font-bold ${isPerfect
              ? "text-green-600"
              : attempt.score > 0
                ? "text-blue-600"
                : "text-slate-500"
      ...
      <div>
        <span
          className={`px-3 py-1 rounded-full text-xs font-bold ${isPerfect
...
const ReadingMultiChoiceMultiAnswer = ({ question, setActiveSpeechQuestion, nextButton, previousButton, shuffleButton }) => {
  ...
  const handleShowAnswer = () => {
    if (!question || !question.correctOptions) return;
      ...
      {showFlashAnswer && (
        ...
        >
          <div className="flex flex-wrap justify-center gap-2">
            {question?.correctOptions?.map((option, index) => (
              <span key={index} className="px-3 py-1.5 rounded-lg bg-green-100 text-green-700 font-bold border border-green-200">
                {option}
              </span>
            ))}
          </div>
