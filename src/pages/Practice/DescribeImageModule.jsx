                ...
                {status !== 'result' ? (
                    ...
                    <div className="flex flex-col md:flex-row h-full">
                        <div className="flex-[1.5] relative p-8 flex items-center justify-center border-r border-slate-700/50">
                            ...
                            <div className={`relative w-full h-full transition-all duration-500 ${status === 'idle' ? 'opacity-30 blur-sm' : 'opacity-100 blur-0'}`}>
                                <img src={question.imageUrl} alt="Task" className="w-full h-full object-contain max-h-[400px] rounded-xl" />
                            </div>

                            {status === 'idle' && (
                                <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                                    <button onClick={handleStartClick} className="w-16 h-16 bg-blue-600 hover:bg-blue-500 text-white rounded-full flex items-center justify-center shadow-2xl transition-all">
                                        <Play size={28} fill="currentColor" className="ml-1" />
                                    </button>
                                    <p className="mt-4 text-white font-bold text-lg">Click To Start</p>
                                </div>
                            )}
                        </div>

                        <div className="flex-1 bg-[#ffffff] p-12 flex flex-col items-center justify-center">
                            {(status === 'prep' || status === 'recording') && (
                                <div className="text-center space-y-8 w-full">
                                    <div className="relative flex items-center justify-center">
                                        <svg className="w-40 h-40 transform -rotate-90">
                                            <circle cx="80" cy="80" r="70" stroke="#333" strokeWidth="6" fill="transparent" />
                                            <circle cx="80" cy="80" r="70" stroke={status === 'prep' ? "#3b82f6" : "#ef4444"} strokeWidth="6" fill="transparent" strokeDasharray={440} strokeDashoffset={440 - (440 * timeLeft) / maxTime} className="transition-all duration-1000 ease-linear" strokeLinecap="round" />
                                        </svg>
                                        <span className="absolute text-5xl font-black text-black">{timeLeft}</span>
                                    </div>
                                    <p className={`text-sm font-bold uppercase tracking-[0.2em] ${status === 'prep' ? 'text-blue-400' : 'text-red-500 animate-pulse'}`}>
                                        {status === 'prep' ? 'Preparation' : 'Recording...'}
                                    </p>
                                    {status === 'prep' ? (
