...
const ManageFIBDragDrops = () => {
    const { user } = useSelector((state) => state.auth);
    const [fibdSections, setFibdSections] = useState([]);
    ...
    const initialForm = {
        title: "",
        ReadingFIBDragDrops: [], // Matches backend schema field for FIBD (Drag Drop)
    };
    ...
    const filteredFIBDSections = useMemo(() => {
        return fibdSections.filter(q => q.title?.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [fibdSections, searchTerm]);

    const getAvailableQuestionsKey = (formQuestionType) => {
        // Backend returns { fibdQuestions: [...] } key in getUnusedFIBDragDropQuestions
        if (formQuestionType === "ReadingFIBDragDrops") {
            return "fibdQuestions";
        }
    ...
    const handleSave = async (e) => {
        ...
        try {
            const payload = {
                title: form.title,
                ReadingFIBDragDrops: form.ReadingFIBDragDrops.map(q => q._id || q),
            };
            ...
            console.error("Error saving FIB Drag Drop section:", err);
        } finally {
    ...
    const handleAddQuestionToForm = (questionType, question) => {
        if (form[questionType].length >= 5) { // Backend limit is 5
            alert("FIB Drag Drop sections cannot have more than 5 questions.");
            return;
        }
        setForm(prevForm => ({
            ...prevForm,
    ...
    const handleEditClick = async (section) => {
        ...
        try {
            ...
            console.error("Failed to fetch FIBD section details for editing:", err);
            setIsModalOpen(false);
        } finally {
            setSubmitLoading(false);
        }
        ...
        return (
            <div className="space-y-4 border p-4 rounded-xl bg-slate-50">
                <h3 className="flex items-center gap-2 font-bold text-slate-700">
                    ...
                    {questionType.replace('ReadingFIBDragDrops', 'FIBD Questions')}
                ...
                <div className="flex flex-wrap gap-2 mb-4 min-h-[40px] border-b pb-2">
                    {form[questionType].length === 0 ? (
                        ...
                        form[questionType].map((q) => (
                            <span key={q._id || q} className="flex items-center gap-1 px-3 py-1 bg-purple-100 text-purple-700 text-xs font-medium rounded-full">
                                {q.title || `ID: ${q._id || q}`}
                                <button type="button" onClick={() => handleRemoveQuestionFromForm(questionType, q._id || q)} className="ml-1 text-purple-500 hover:text-purple-800"><X size={14} /></button>
                            </span>
                        ))
                    )}
                </div>
                        ...
                        {questionsToDisplay.length === 0 ? (
                            ...
                            questionsToDisplay.map((q) => (
                                <button
                                    ...
                                    className="w-full text-left p-2 border rounded-lg hover:bg-purple-50 transition-colors text-sm text-slate-700"
                                >
                                    {q.title || `ID: ${q._id}`}
                                </button>
                            ))
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    <div className="space-y-1">
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">Manage FIB Drag Drop Sections</h2>
                        <p className="text-slate-500 font-medium">Create and organize PTE Reading Fill in the Blanks (Drag & Drop) sections</p>
                    </div>
                    <motion.button
                        ...
                        className="flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-purple-100"
                    >
                        <PlusCircle size={22} /> <span>Create New FIBD Section</span>
                    </motion.button>
                </div>

                {/* SEARCH */}
                <div className="relative group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-purple-500" size={20} />
                    <input
                        type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                        placeholder="Search by section title..."
                        className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 bg-white focus:ring-4 focus:ring-purple-50 outline-none transition-all"
                    />
                </div>
                ...
                <div className="space-y-4">
                    ...
                    ) : filteredFIBDSections.length === 0 ? (
                        <div className="py-20 text-center text-slate-500">No FIBD sections found.</div>
                    ) : (
                        filteredFIBDSections.map((section) => (
                            <motion.div key={section._id} whileHover={{ y: -4 }} className="grid grid-cols-1 md:grid-cols-12 items-center bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:border-purple-200 transition-all group">
                                <div className="col-span-6 flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Reading Module - FIB Drag Drop</p>
                                    </div>
                                </div>
                                <div className="col-span-3 text-center text-slate-600 text-sm">
                                    <p>Questions: {section.ReadingFIBDragDrops?.length || 0}</p>
                                </div>
                                <div className="col-span-3 flex justify-end gap-2">
                                    ...
                                    <button onClick={() => handleEditClick(section)} className="p-2 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg"><Edit size={18} /></button>
                        ...
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                            ...
                            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                                <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                                    <h2 className="text-2xl font-black text-slate-800">{editingId ? "Edit FIB Drag Drop Section" : "New FIB Drag Drop Section"}</h2>
                                ...
                                <form onSubmit={handleSave} className="p-8 overflow-y-auto space-y-6">
                                    ...
                                    {submitLoading ? (
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
                                            {renderQuestionSelection("ReadingFIBDragDrops", MousePointer2)}
                                        </div>
                                    )}

                                    <button disabled={submitLoading || form.ReadingFIBDragDrops.length > 5} className="w-full py-4 bg-purple-600 text-white rounded-2xl font-bold shadow-xl">
                                        {submitLoading ? <Loader2 className="animate-spin mx-auto" /> : "Save FIB Drag Drop Section"}
                                    </button>
                                    {form.ReadingFIBDragDrops.length > 5 && (
                            ...
                            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto">
                                <div className="p-10 space-y-8">
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <span className="bg-purple-100 text-purple-700 text-[10px] font-black px-3 py-1 rounded-full uppercase">FIBD Section</span>
                                    ...
                                    {[
                                        { type: "ReadingFIBDragDrops", label: "FIB Drag Drop Questions", icon: MousePointer2 },
                                        ...
                                        viewData[type] && viewData[type].length > 0 && (
                                            <div key={index} className="space-y-4">
                                                <h3 className="flex items-center gap-2 text-xl font-bold text-slate-800 border-b pb-2">
                                                    <Icon className="text-purple-500" size={24} /> {label} ({viewData[type].length})
                                                </h3>
...
export default ManageFIBDragDrops;
