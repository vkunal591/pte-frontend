...
const ManageRepeatSentences = () => {
  const { user } = useSelector((state) => state.auth);
  const [rsSections, setRsSections] = useState([]); // State for Repeat Sentence sections
  ...
  const [availableQuestions, setAvailableQuestions] = useState({}); // Stores available Repeat Questions
  const [unusedLoading, setUnusedLoading] = useState(true);

  // State for form fields
  const initialForm = {
    title: "",
    repeatSentenceQuestions: [], // Only one type of question for RS sections
  };
  ...
  const filteredRSSections = useMemo(() => {
    return rsSections.filter(q => q.title?.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [rsSections, searchTerm]);

  const getAvailableQuestionsKey = (formQuestionType) => {
    return formQuestionType.replace('Questions', ''); // "repeatSentenceQuestions" -> "repeatSentence"
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      console.error("Error saving Repeat Sentence section:", err);
    } finally {
  ...
  const handleEditClick = async (section) => {
    ...
    try {
      ...
      console.error("Failed to fetch Repeat Sentence section details for editing:", err);
      setIsModalOpen(false);
    } finally {
      setSubmitLoading(false);
    }
    ...
    return (
      <div className="space-y-4 border p-4 rounded-xl bg-slate-50">
        <h3 className="flex items-center gap-2 font-bold text-slate-700">
          <Icon size={20} className="text-lime-500" /> {/* Lime color for RS */}
          ...
          {form[questionType].length === 0 ? (
            ...
            form[questionType].map((q) => (
              <span key={q._id || q} className="flex items-center gap-1 px-3 py-1 bg-lime-100 text-lime-700 text-xs font-medium rounded-full"> {/* Lime color */}
                ...
                <button type="button" onClick={() => handleRemoveQuestionFromForm(questionType, q._id || q)} className="ml-1 text-lime-500 hover:text-lime-800"><X size={14} /></button> {/* Lime color */}
              </span>
            ))
          )}
        </div>
            ...
            {questionsToDisplay.length === 0 ? (
              ...
              questionsToDisplay.map((q) => (
                <button
                  ...
                  className="w-full text-left p-2 border rounded-lg hover:bg-lime-50 transition-colors text-sm text-slate-700" // Lime hover color
                >
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <div className="space-y-1">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Manage Repeat Sentence Sections</h2>
            <p className="text-slate-500 font-medium">Create and organize PTE Repeat Sentence test sections</p>
          </div>
          <motion.button
            ...
            className="flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-lime-600 to-green-600 text-white rounded-2xl font-bold shadow-lg shadow-lime-100" // Lime gradient
          >
            <PlusCircle size={22} /> <span>Create New RS Section</span>
          </motion.button>
        </div>

        {/* SEARCH */}
        <div className="relative group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-lime-500" size={20} /> {/* Lime color */}
          <input
            type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search by section title..."
            className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 bg-white focus:ring-4 focus:ring-lime-50 outline-none transition-all" // Lime ring focus
          />
        </div>

        {/* LIST OF REPEAT SENTENCE SECTIONS */}
        <div className="space-y-4">
          {loading ? (
            <div className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-lime-500" size={40} /></div> // Lime loader
          ) : filteredRSSections.length === 0 ? (
            <div className="py-20 text-center text-slate-500">No Repeat Sentence sections found.</div>
          ) : (
            filteredRSSections.map((section) => (
              <motion.div key={section._id} whileHover={{ y: -4 }} className="grid grid-cols-1 md:grid-cols-12 items-center bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:border-lime-200 transition-all group"> {/* Lime hover border */}
                <div className="col-span-6 flex items-center gap-5">
                  <div className="w-12 h-12 rounded-xl bg-lime-50 text-lime-600 flex items-center justify-center group-hover:bg-lime-600 group-hover:text-white transition-all"> {/* Lime accent */}
                    <Headphones size={20} /> {/* Icon for listening task */}
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-800">{section.title}</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Speaking Module - Repeat Sentence</p>
                  </div>
                </div>
                <div className="col-span-3 text-center text-slate-600 text-sm">
                  <p>Questions: {section.repeatSentenceQuestions?.length || 0}</p>
                </div>
                <div className="col-span-3 flex justify-end gap-2">
                  <button onClick={() => { setViewData(section); setIsViewModalOpen(true); }} className="p-2 text-slate-400 hover:text-lime-600 hover:bg-lime-50 rounded-lg"><Eye size={18} /></button> {/* Lime hover */}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                  <h2 className="text-2xl font-black text-slate-800">{editingId ? "Edit Repeat Sentence Section" : "New Repeat Sentence Section"}</h2>
                ...
                <form onSubmit={handleSave} className="p-8 overflow-y-auto space-y-6">
                  ...
                  <button disabled={submitLoading} className="w-full py-4 bg-lime-600 text-white rounded-2xl font-bold shadow-xl"> {/* Lime button */}
                    {submitLoading ? <Loader2 className="animate-spin mx-auto" /> : "Save Repeat Sentence Section"}
                  </button>
                </form>
              </motion.div>
            </div>
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto">
                <div className="p-10 space-y-8">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <span className="bg-lime-100 text-lime-700 text-[10px] font-black px-3 py-1 rounded-full uppercase">Repeat Sentence Section</span> {/* Lime tag */}
                  ...
                  {[
                    { type: "repeatSentenceQuestions", label: "Repeat Sentence Questions", icon: Headphones },
                    ...
                    viewData[type] && viewData[type].length > 0 && (
                      <div key={index} className="space-y-4">
                        <h3 className="flex items-center gap-2 text-xl font-bold text-slate-800 border-b pb-2">
                          <Icon className="text-lime-500" size={24} /> {label} ({viewData[type].length}) {/* Lime icon */}
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {viewData[type].map(q => (
                            <div key={q} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                              ...
                              {q.audioUrl && ( // Assuming RepeatQuestion has an audioUrl
...
export default ManageRepeatSentences;