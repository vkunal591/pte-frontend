...
const ManageSummarizeGroupDiscussions = () => {
  const { user } = useSelector((state) => state.auth);
  const [sgdSections, setSgdSections] = useState([]); // State for SGD sections
  ...
  const [availableQuestions, setAvailableQuestions] = useState({}); // Stores available SGD Questions
  const [unusedLoading, setUnusedLoading] = useState(true);

  // State for form fields
  const initialForm = {
    title: "",
    summarizeGroupDiscussionQuestions: [], // Only one type of question for SGD sections
  };
  ...
  const filteredSGDSections = useMemo(() => {
    return sgdSections.filter(q => q.title?.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [sgdSections, searchTerm]);

  const getAvailableQuestionsKey = (formQuestionType) => {
    return formQuestionType.replace('Questions', ''); // "summarizeGroupDiscussionQuestions" -> "summarizeGroupDiscussion"
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        title: form.title,
        summarizeGroupDiscussionQuestions: form.summarizeGroupDiscussionQuestions.map(q => q._id || q),
      };
      ...
      console.error("Error saving Summarize Group Discussion section:", err);
    } finally {
  ...
  const handleEditClick = async (section) => {
    ...
    try {
      ...
      console.error("Failed to fetch Summarize Group Discussion section details for editing:", err);
      setIsModalOpen(false);
    } finally {
      setSubmitLoading(false);
    }
    ...
    return (
      <div className="space-y-4 border p-4 rounded-xl bg-slate-50">
        <h3 className="flex items-center gap-2 font-bold text-slate-700">
          <Icon size={20} className="text-orange-500" /> {/* Orange color for SGD */}
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
          <div className="space-y-1">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Manage Summarize Group Discussion Sections</h2>
            <p className="text-slate-500 font-medium">Create and organize PTE Summarize Group Discussion test sections</p>
          </div>
          <motion.button
          ...
          >
            <PlusCircle size={22} /> <span>Create New SGD Section</span>
          </motion.button>
        </div>
        ...
        {/* LIST OF SGD SECTIONS */}
        <div className="space-y-4">
          {loading ? (
            <div className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-orange-500" size={40} /></div> // Orange loader
          ) : filteredSGDSections.length === 0 ? (
            <div className="py-20 text-center text-slate-500">No Summarize Group Discussion sections found.</div>
          ) : (
            filteredSGDSections.map((section) => (
              <motion.div key={section._id} whileHover={{ y: -4 }} className="grid grid-cols-1 md:grid-cols-12 items-center bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:border-orange-200 transition-all group"> {/* Orange hover border */}
                <div className="col-span-6 flex items-center gap-5">
                  <div className="w-12 h-12 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition-all"> {/* Orange accent */}
                    <Users size={20} /> {/* Icon for group discussion */}
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-800">{section.title}</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Speaking Module - Summarize Group Discussion</p>
                  </div>
                </div>
                <div className="col-span-3 text-center text-slate-600 text-sm">
                  <p>Questions: {section.summarizeGroupDiscussionQuestions?.length || 0}</p>
                </div>
                <div className="col-span-3 flex justify-end gap-2">
                  <button onClick={() => { setViewData(section); setIsViewModalOpen(true); }} className="p-2 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg"><Eye size={18} /></button> {/* Orange hover */}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                  <h2 className="text-2xl font-black text-slate-800">{editingId ? "Edit Summarize Group Discussion Section" : "New Summarize Group Discussion Section"}</h2>
                ...
                <form onSubmit={handleSave} className="p-8 overflow-y-auto space-y-6">
                  ...
                  <button disabled={submitLoading} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-bold shadow-xl"> {/* Orange button */}
                    {submitLoading ? <Loader2 className="animate-spin mx-auto" /> : "Save Summarize Group Discussion Section"}
                  </button>
                </form>
              </motion.div>
            </div>
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto">
                <div className="p-10 space-y-8">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <span className="bg-orange-100 text-orange-700 text-[10px] font-black px-3 py-1 rounded-full uppercase">Summarize Group Discussion Section</span> {/* Orange tag */}
                  ...
                  {[
                    { type: "summarizeGroupDiscussionQuestions", label: "Summarize Group Discussion Questions", icon: MessageSquare },
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {viewData[type].map(q => (
                            <div key={q} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                              ...
                              {q.audioUrl && ( // Assuming SummarizeGroupQuestion has an audioUrl for the discussion
...
export default ManageSummarizeGroupDiscussions;