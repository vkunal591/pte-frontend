...
const ManageROs = () => {
    ...
    const [roSections, setBRSections] = useState([]);
    ...
    const initialForm = {
        title: "",
        reorderQuestions: [], // Matches backend schema field for RO
    };
    ...
    const filteredROSections = useMemo(() => {
        return roSections.filter(q => q.title?.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [roSections, searchTerm]);

    const getAvailableQuestionsKey = (formQuestionType) => {
        if (formQuestionType === "reorderQuestions") {
            return "reorderQuestions"; // Key returned by backend
        }
    ...
    const handleSave = async (e) => {
        ...
        try {
            const payload = {
                title: form.title,
                reorderQuestions: form.reorderQuestions.map(q => q._id || q),
            };
            ...
            console.error("Error saving RO section:", err);
        } finally {
    ...
    const handleAddQuestionToForm = (questionType, question) => {
        if (form[questionType].length >= 5) { // Backend limit is 5
            alert("RO sections cannot have more than 5 questions.");
            return;
        }
        setForm(prevForm => ({
            ...prevForm,
    ...
    const handleEditClick = async (section) => {
        ...
        try {
            ...
            const formQuestionType = "reorderQuestions";
            ...
            console.error("Failed to fetch RO section details for editing:", err);
            setIsModalOpen(false);
        } finally {
            setSubmitLoading(false);
        }
        ...
        return (
            <div className="space-y-4 border p-4 rounded-xl bg-slate-50">
                <h3 className="flex items-center gap-2 font-bold text-slate-700">
                    ...
                    {questionType.replace('reorderQuestions', 'RO Questions')}
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    <div className="space-y-1">
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">Manage RO Sections</h2>
                        <p className="text-slate-500 font-medium">Create and organize PTE Reorder Paragraph sections</p>
                    </div>
                    <motion.button
                        ...
                        className="flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-amber-600 to-orange-600 text-white rounded-2xl font-bold shadow-lg shadow-amber-100"
                    >
                        <PlusCircle size={22} /> <span>Create New RO Section</span>
                    </motion.button>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    ) : filteredROSections.length === 0 ? (
                        <div className="py-20 text-center text-slate-500">No RO sections found.</div>
                    ) : (
                        filteredROSections.map((section) => (
                            <motion.div key={section._id} whileHover={{ y: -4 }} className="grid grid-cols-1 md:grid-cols-12 items-center bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:border-amber-200 transition-all group">
                                <div className="col-span-6 flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Reading Module - RO</p>
                                    </div>
                                </div>
                                <div className="col-span-3 text-center text-slate-600 text-sm">
                                    <p>Questions: {section.reorderQuestions?.length || 0}</p>
                                </div>
                        ...
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                            ...
                            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                                <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
                                    <h2 className="text-2xl font-black text-slate-800">{editingId ? "Edit RO Section" : "New RO Section"}</h2>
                                ...
                                <form onSubmit={handleSave} className="p-8 overflow-y-auto space-y-6">
                                    ...
                                    {submitLoading ? (
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
                                            {renderQuestionSelection("reorderQuestions", ArrowUpDown)}
                                        </div>
                                    )}

                                    <button disabled={submitLoading || form.reorderQuestions.length > 5} className="w-full py-4 bg-amber-600 text-white rounded-2xl font-bold shadow-xl">
                                        {submitLoading ? <Loader2 className="animate-spin mx-auto" /> : "Save RO Section"}
                                    </button>
                                    {form.reorderQuestions.length > 5 && (
                            ...
                            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto">
                                <div className="p-10 space-y-8">
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <span className="bg-amber-100 text-amber-700 text-[10px] font-black px-3 py-1 rounded-full uppercase">RO Section</span>
                                    ...
                                    {[
                                        { type: "reorderQuestions", label: "RO Questions", icon: ArrowUpDown },
