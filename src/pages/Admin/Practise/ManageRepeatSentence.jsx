...
const initialForm = {
    title: '',
    transcript: '',
    difficulty: 'Medium', // Repeat Sentence doesn't typically have difficulty in PTE, but kept for consistency
    prepareTime: 3,  // Typical prepare time for Repeat Sentence
    answerTime: 15,  // Typical answer time for Repeat Sentence
    audio: null,
...
const ManageRepeatSentence = () => {
    ...
    const fetchQuestions = async () => {
        ...
        try {
            ...
            const res = await axios.get(`/api/repeat-sentence/all`); 
    ...
    const filteredQuestions = useMemo(() => {
        return questions.filter(q => 
            ...
            (q.transcript && q.transcript.toLowerCase().includes(searchTerm.toLowerCase()))
        );
    ...
    const openEditModal = (q) => {
        setForm({
            ...
            audio: null, // Audio isn't typically edited directly here without re-uploading
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.title || (!form.audio && !editingId) || !form.transcript) { // Audio not required if editing and not changing
            alert("Question Title, Transcript, and Audio file are required for new questions.");
            return;
        }
        ...
        <AdminLayout>
            <div className="p-8 bg-[#f8fafc] min-h-screen">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900">Manage Repeat Sentence</h1>
                        <p className="text-slate-500 mt-1">Upload and manage audio sentence questions for practice</p>
                    </div>
                    <motion.button
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Question Details</th>
                                    <th className="px-6 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Transcript Preview</th>
                                ...
                                {loading ? (
                                    ...
                                    filteredQuestions.map((q) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <p className="text-sm text-slate-600 line-clamp-2 max-w-xs">{q.transcript}</p>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex justify-end gap-2">
                                                    <ActionButton onClick={() => handleView(q)} icon={<Eye size={18}/>} />
                                    ...
                                    <div className="relative group">
                                        ...
                                        >
                                            ...
                                            <span className="text-sm font-medium text-slate-600">
                                                {form.audio ? form.audio.name : (editingId ? "Click to change audio (optional)" : "Click to upload audio")}
                                            </span>
                            ...
                            >
                                ...
                                <div className="p-8 space-y-6">
                                    <div className="space-y-2">
                                        <span className="text-indigo-400 text-sm font-bold tracking-widest uppercase">Preview Repeat Sentence Question</span>
