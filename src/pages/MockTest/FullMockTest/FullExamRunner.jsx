...
export default function FullExamRunner() {
    ...
    const handleListeningComplete = async (answers) => {
        ...
        try {
            ...
            // USE API INSTANCE (Includes Credentials/Cookies)
            // relative path since baseURL handles localhost:5000/api
            const res = await api.post(`/mocktest/full/${id}/submit`, formData);
    ...
    /* --- DATA MAPPING --- */
    const speakingData = testData ? {
        ...testData.speaking,
        _id: testData._id,
        readAloudQuestions: testData.speaking.readAloud,
        repeatSentenceQuestions: testData.speaking.repeatSentence,
        describeImageQuestions: testData.speaking.describeImage,
        reTellLectureQuestions: testData.speaking.reTellLecture,
        summarizeSpokenTextQuestions: []
    } : null;

    const writingData = testData ? {
        ...testData.writing,
        _id: testData._id,
        summarizeWrittenText: testData.writing.summarizeWrittenText,
        writeEssay: testData.writing.writeEssay,
        summarizeSpokenText: [],
        writeFromDictation: []
    } : null;

    const readingData = testData ? {
        ...testData.reading,
        _id: testData._id,
        summarizeWrittenText: [],
        multipleChoiceMultiple: testData.reading.multipleChoiceMultiple,
        reOrderParagraphs: testData.reading.reOrderParagraphs,
        fillInTheBlanksWithDragDrop: testData.reading.fillInTheBlanksDragDrop,
        multipleChoiceSingle: testData.reading.multipleChoiceSingle
    } : null;

    const listeningData = testData ? {
        ...testData.listening,
        _id: testData._id,
        summarizeSpokenTextQuestions: testData.listening.summarizeSpokenText,
        multipleChoiceMultiple: testData.listening.multipleChoiceMultiple,
        fillInTheBlanks: testData.listening.fillInTheBlanks,
        highlightIncorrectSummary: testData.listening.highlightCorrectSummary,
        multipleChoiceSingle: testData.listening.multipleChoiceSingle,
        selectMissingWord: testData.listening.selectMissingWord,
        highLightIncorrectWords: testData.listening.highlightIncorrectWords,
        writeFromDictation: testData.listening.writeFromDictation,
        answerShortQuestion: testData.speaking.answerShortQuestion
    } : null;
    ...
    return (
        <div className="w-full min-h-screen bg-gray-50 flex flex-col">
            ...
            {currentStep === STEPS.FINISHED && (
                testResult ? (
                    <FullMockTestResult result={testResult} />
                ) : (
